...
function ENT:Use(ply)
	...
	if( (ply:GetNWInt(ply:Nick() .. "_farms") + ply:GetNWInt(ply:Nick() .. "_mines")) < ( ply:GetNWInt(ply:Nick() .. "_cities") + 2) ) then
		if(!self.claimed) then
			...
			ply:SetNWInt(ply:Nick() .. "_farms", ply:GetNWInt(ply:Nick() .. "_farms") + 1) 
			self.claimed = true
			self.Overlord = ply
		end
	end
...
function ENT:RemoveControl()
	if(self.Overlord != "nothing") then
		self.Overlord:SetNWInt(self.Overlord:Nick() .. "_farms", self.Overlord:GetNWInt(self.Overlord:Nick() .. "_farms") - 1)
		self.claimed = false
		self.Overlord = "nothing"
		self.Entity:SetColor( 255,255, 255,255 )
		self.health = 250
